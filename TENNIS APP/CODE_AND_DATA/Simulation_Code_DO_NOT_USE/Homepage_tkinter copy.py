...
from collect_and_process import start_live_processing, stop_live_processing
from collect_and_process import main as measurement_main  # Import measurement function
...
def run_calibration():
    ...
    def calibration_task():
        ...
        calculated_offsets = {"Offset X": Offset_x_total, "Offset Y": Offset_y_total, "Offset Z": Offset_z_total}
...
def run_measurement():
    ...
    if not measurement_running:
        ...
        threading.Thread(target=measurement_main).start()
...
def update_evaluation(angle, acceleration, evaluation):
    # Add the new result to the history
    evaluation_history.appendleft({"angle": angle, "acceleration": acceleration, "evaluation": evaluation})

    # Update the evaluation display
    for widget in evaluation_frame.winfo_children():
        widget.destroy()  # Clear the current content in the frame

    # Display the last 5 evaluation results
    for i, result in enumerate(evaluation_history):
        # Create the label for angle, acceleration, and evaluation
        angle_label = ttk.Label(evaluation_frame, text=f"Angle: {result['angle']:.2f}°")
        angle_label.grid(row=i, column=0, sticky="w", padx=10, pady=5)

        acceleration_label = ttk.Label(evaluation_frame, text=f"Acceleration: {result['acceleration']:.2f} m/s²")
        acceleration_label.grid(row=i, column=1, sticky="w", padx=10, pady=5)

        evaluation_label = ttk.Label(evaluation_frame, text=f"Evaluation: {result['evaluation']}", 
                                     foreground="green" if result['evaluation'] == "good" else "red")
        evaluation_label.grid(row=i, column=2, sticky="w", padx=10, pady=5)
...
# Load logo
try:
    logo_image = Image.open("..\CODE\FINAL_CODE\Tennis Shot Analyzer.jpeg")  # Replace with the path to your logo
...
measurement_button = ttk.Button(buttons_frame, text="Start Measurement", command=start_live_processing)
measurement_button.pack(side=tk.LEFT, padx=10)

stop_button = ttk.Button(buttons_frame, text="Stop Measurement", command=stop_live_processing)
